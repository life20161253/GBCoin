...
static const char *strMainNetDNSSeed[][2] = {
    ...
    //{"205.209.156.145", "205.209.156.145"},
    {"205.209.147.150", "205.209.147.150"},
...
void ThreadDNSAddressSeed()
{
    ...
    for (unsigned int seed_idx = 0; strDNSSeed[seed_idx][0] != NULL; seed_idx++) {
        if (HaveNameProxy()) {
            ...
            printf("add a shot=%s at ThreadDNSAddressSeed.\n", strDNSSeed[seed_idx][1]);
        } else {
            ...
            if (LookupHost(strDNSSeed[seed_idx][1], vaddr))
            {
                BOOST_FOREACH(CNetAddr& ip, vaddr)
                {
                    ...
                    vAdd.push_back(addr);
                    found++;
                    printf("find an addresses=%s from DNS sees and shot1=%s when seed_idx=%d at ThreadDNSAddressSeed.\n",
                           ip.ToString().c_str(), strDNSSeed[seed_idx][1], seed_idx);
                }
            }
...
{
    printf("start ThreadOpenConnections().\n");
...
{
    printf("start ThreadOpenAddedConnections().\n");
    {
        LOCK(cs_vAddedNodes);
