...
// Copyright (c) 2013 The GBCoin developers
...
uint256 hashGenesisBlock("0x00000c20318de1811285ffb33ef5064a434ff75dc58db9c420bcf7359eeb145a");
static const unsigned int timeGenesisBlock = 1468504515;
...
bool CTransaction::CheckTransaction(CValidationState &state) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
        ...
        if (txout.nValue > (MAX_MONEY-MONEY_RESERVED))
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
       const char* pszTimestamp = "already in already databases we GBCoinpark from USA with Transactions.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.print();
        assert(block.hashMerkleRoot == uint256("0xf064ab0787c58a27cfd6b9738bcae7a2308a62f96a7416cb8c21ae8be40079b0"));
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GBCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GBCoinMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        if (!pblocktemplate.get())
        {
            //printf("**************Running GBCoinMiner with outloop not get block at time=%d.\n", GetTime());
            return;
        }
        CBlock *pblock = &pblocktemplate->block;
        IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

        printf("Running GBCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
//            unsigned int nNonceFound;
        ...
        // disable in testing
            ...
            //printf("**************Running GBCoinMiner with inside loop at time=%d.\n", GetTime());
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    //nThreads = 1;   // for test, by Ligo, 2016.6.5
    printf("miner thread count=%d.\n", nThreads);
